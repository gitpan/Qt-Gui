################################################################
# THE FOLLOWING CODE IS AUTOMATED, ANY MODIFICATION WILL BE LOST!
#
# Copyright (C) 2007 - 2012 by Dongxu Ma <dongxu _at_ cpan _dot_ org>
#
# This library is free software; you can redistribute it and/or 
# modify it under the same terms as Perl itself.
# See http://dev.perl.org/licenses/artistic.html
################################################################

MODULE = Qt::Gui			PACKAGE = Qt::Gui::Template::T016
PROTOTYPES: DISABLE

# classname: QVector<QTextLength>
################################################################
#### 
#### PUBLIC MEMBER FUNCTIONS
#### 
################################################################

##  QVector<QTextLength>()
##  QVector<QTextLength>(int size)
##  QVector<QTextLength>(const QVector<QTextLength> & v)
##  QVector<QTextLength>(int size, const QTextLength & t)
  void
T016::new(...)
PREINIT:
QVector<QTextLength> *ret;
int arg10;
QVector<QTextLength> * arg20;
int arg30;
QTextLength * arg31;
PPCODE:
    switch(items) {
      case 1:
      {
        if (1) {
      
    ret = new QVector<QTextLength>();
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
        break;
      }
      case 2:
      {
        if (SvIOK(ST(1))) {
      arg10 = (int)SvIV(ST(1));
    ret = new QVector<QTextLength>(arg10);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
        else if (sv_isa(ST(1), "Qt::Gui::Template::T016")) {
      arg20 = reinterpret_cast<QVector<QTextLength> *>(SvIV((SV*)SvRV(ST(1))));
    ret = new QVector<QTextLength>(*arg20);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 3:
      {
        if (SvIOK(ST(1)) && sv_isa(ST(2), "Qt::Gui::QTextLength")) {
      arg30 = (int)SvIV(ST(1));
      arg31 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(2))));
    ret = new QVector<QTextLength>(arg30, *arg31);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

##  ~QVector<QTextLength>()
void
T016::DESTROY()
CODE:
    if(THIS != 0 && !SvREADONLY(SvRV(ST(0))))
        delete THIS;

## void append(const QTextLength & t)
void
T016::append(...)
PREINIT:
QTextLength * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    (void)THIS->append(*arg00);
    XSRETURN(0);
    }

## int capacity()
void
T016::capacity(...)
PREINIT:
PPCODE:
    if (1) {
      
    int ret = THIS->capacity();
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }

## void clear()
void
T016::clear(...)
PREINIT:
PPCODE:
    if (1) {
      
    (void)THIS->clear();
    XSRETURN(0);
    }

## const QTextLength * constData()
void
T016::constData(...)
PREINIT:
PPCODE:
    if (1) {
      
    const QTextLength * ret = THIS->constData();
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::QTextLength", (void *)ret);
    XSRETURN(1);
    }

## bool contains(const QTextLength & t)
void
T016::contains(...)
PREINIT:
QTextLength * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    bool ret = THIS->contains(*arg00);
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## int count(const QTextLength & t)
void
T016::count(...)
PREINIT:
QTextLength * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    int ret = THIS->count(*arg00);
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }

## const QTextLength * data()
void
T016::data(...)
PREINIT:
PPCODE:
    if (1) {
      
    const QTextLength * ret = THIS->data();
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::QTextLength", (void *)ret);
    XSRETURN(1);
    }

## void detach()
void
T016::detach(...)
PREINIT:
PPCODE:
    if (1) {
      
    (void)THIS->detach();
    XSRETURN(0);
    }

## QVector<QTextLength> & fill(const QTextLength & t, int size)
## QVector<QTextLength> & fill(const QTextLength & t, int size = -1)
void
T016::fill(...)
PREINIT:
QTextLength * arg00;
int arg01;
QTextLength * arg10;
int arg11 = -1;
PPCODE:
    switch(items) {
      case 2:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg10 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    QVector<QTextLength> * ret = &THIS->fill(*arg10, arg11);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 3:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength") && SvIOK(ST(2))) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
      arg01 = (int)SvIV(ST(2));
    QVector<QTextLength> * ret = &THIS->fill(*arg00, arg01);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

## int indexOf(const QTextLength & t, int from)
## int indexOf(const QTextLength & t, int from = 0)
void
T016::indexOf(...)
PREINIT:
QTextLength * arg00;
int arg01;
QTextLength * arg10;
int arg11 = 0;
PPCODE:
    switch(items) {
      case 2:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg10 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    int ret = THIS->indexOf(*arg10, arg11);
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 3:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength") && SvIOK(ST(2))) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
      arg01 = (int)SvIV(ST(2));
    int ret = THIS->indexOf(*arg00, arg01);
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

## void insert(int i, const QTextLength & t)
## void insert(int i, int n, const QTextLength & t)
void
T016::insert(...)
PREINIT:
int arg00;
QTextLength * arg01;
int arg10;
int arg11;
QTextLength * arg12;
PPCODE:
    switch(items) {
      case 3:
      {
        if (SvIOK(ST(1)) && sv_isa(ST(2), "Qt::Gui::QTextLength")) {
      arg00 = (int)SvIV(ST(1));
      arg01 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(2))));
    (void)THIS->insert(arg00, *arg01);
    XSRETURN(0);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 4:
      {
        if (SvIOK(ST(1)) && SvIOK(ST(2)) && sv_isa(ST(3), "Qt::Gui::QTextLength")) {
      arg10 = (int)SvIV(ST(1));
      arg11 = (int)SvIV(ST(2));
      arg12 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(3))));
    (void)THIS->insert(arg10, arg11, *arg12);
    XSRETURN(0);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

## bool isDetached()
void
T016::isDetached(...)
PREINIT:
PPCODE:
    if (1) {
      
    bool ret = THIS->isDetached();
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## bool isEmpty()
void
T016::isEmpty(...)
PREINIT:
PPCODE:
    if (1) {
      
    bool ret = THIS->isEmpty();
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## bool isSharedWith(const QVector<QTextLength> & other)
void
T016::isSharedWith(...)
PREINIT:
QVector<QTextLength> * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::Template::T016")) {
      arg00 = reinterpret_cast<QVector<QTextLength> *>(SvIV((SV*)SvRV(ST(1))));
    bool ret = THIS->isSharedWith(*arg00);
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## int lastIndexOf(const QTextLength & t, int from)
## int lastIndexOf(const QTextLength & t, int from = -1)
void
T016::lastIndexOf(...)
PREINIT:
QTextLength * arg00;
int arg01;
QTextLength * arg10;
int arg11 = -1;
PPCODE:
    switch(items) {
      case 2:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg10 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    int ret = THIS->lastIndexOf(*arg10, arg11);
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 3:
      {
        if (sv_isa(ST(1), "Qt::Gui::QTextLength") && SvIOK(ST(2))) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
      arg01 = (int)SvIV(ST(2));
    int ret = THIS->lastIndexOf(*arg00, arg01);
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

## bool operator!=(const QVector<QTextLength> & v)
void
T016::operator_not_equal(...)
PREINIT:
QVector<QTextLength> * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::Template::T016")) {
      arg00 = reinterpret_cast<QVector<QTextLength> *>(SvIV((SV*)SvRV(ST(1))));
    bool ret = THIS->operator!=(*arg00);
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## QVector<QTextLength> & operator=(const QVector<QTextLength> & v)
void
T016::operator_assign(...)
PREINIT:
QVector<QTextLength> * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::Template::T016")) {
      arg00 = reinterpret_cast<QVector<QTextLength> *>(SvIV((SV*)SvRV(ST(1))));
    QVector<QTextLength> * ret = &THIS->operator=(*arg00);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::Template::T016", (void *)ret);
    XSRETURN(1);
    }

## bool operator==(const QVector<QTextLength> & v)
void
T016::operator_equal_to(...)
PREINIT:
QVector<QTextLength> * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::Template::T016")) {
      arg00 = reinterpret_cast<QVector<QTextLength> *>(SvIV((SV*)SvRV(ST(1))));
    bool ret = THIS->operator==(*arg00);
    ST(0) = sv_newmortal();
    ST(0) = boolSV(ret);
    XSRETURN(1);
    }

## const QTextLength & operator[](int i)
void
T016::operator_array(...)
PREINIT:
int arg00;
PPCODE:
    if (SvIOK(ST(1))) {
      arg00 = (int)SvIV(ST(1));
    const QTextLength * ret = &THIS->operator[](arg00);
    ST(0) = sv_newmortal();
    sv_setref_pv(ST(0), "Qt::Gui::QTextLength", (void *)ret);
    XSRETURN(1);
    }

## void prepend(const QTextLength & t)
void
T016::prepend(...)
PREINIT:
QTextLength * arg00;
PPCODE:
    if (sv_isa(ST(1), "Qt::Gui::QTextLength")) {
      arg00 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(1))));
    (void)THIS->prepend(*arg00);
    XSRETURN(0);
    }

## void remove(int i)
## void remove(int i, int n)
void
T016::remove(...)
PREINIT:
int arg00;
int arg10;
int arg11;
PPCODE:
    switch(items) {
      case 2:
      {
        if (SvIOK(ST(1))) {
      arg00 = (int)SvIV(ST(1));
    (void)THIS->remove(arg00);
    XSRETURN(0);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      case 3:
      {
        if (SvIOK(ST(1)) && SvIOK(ST(2))) {
      arg10 = (int)SvIV(ST(1));
      arg11 = (int)SvIV(ST(2));
    (void)THIS->remove(arg10, arg11);
    XSRETURN(0);
    }
	else
            Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
      }
      default:
        Perl_croak(aTHX_ "wrong number/type of arguments passed in");
        break;
    }

## void replace(int i, const QTextLength & t)
void
T016::replace(...)
PREINIT:
int arg00;
QTextLength * arg01;
PPCODE:
    if (SvIOK(ST(1)) && sv_isa(ST(2), "Qt::Gui::QTextLength")) {
      arg00 = (int)SvIV(ST(1));
      arg01 = reinterpret_cast<QTextLength *>(SvIV((SV*)SvRV(ST(2))));
    (void)THIS->replace(arg00, *arg01);
    XSRETURN(0);
    }

## void reserve(int size)
void
T016::reserve(...)
PREINIT:
int arg00;
PPCODE:
    if (SvIOK(ST(1))) {
      arg00 = (int)SvIV(ST(1));
    (void)THIS->reserve(arg00);
    XSRETURN(0);
    }

## void resize(int size)
void
T016::resize(...)
PREINIT:
int arg00;
PPCODE:
    if (SvIOK(ST(1))) {
      arg00 = (int)SvIV(ST(1));
    (void)THIS->resize(arg00);
    XSRETURN(0);
    }

## void setSharable(bool shrable)
void
T016::setSharable(...)
PREINIT:
bool arg00;
PPCODE:
    if (1) {
      arg00 = (bool)SvTRUE(ST(1));
    (void)THIS->setSharable(arg00);
    XSRETURN(0);
    }

## int size()
void
T016::size(...)
PREINIT:
PPCODE:
    if (1) {
      
    int ret = THIS->size();
    ST(0) = sv_newmortal();
    sv_setiv(ST(0), (IV)ret);
    XSRETURN(1);
    }

## void squeeze()
void
T016::squeeze(...)
PREINIT:
PPCODE:
    if (1) {
      
    (void)THIS->squeeze();
    XSRETURN(0);
    }
